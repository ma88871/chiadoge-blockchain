...
from chiadoge.consensus.cost_calculator import calculate_cost_of_program, NPCResult
from chiadoge.full_node.bundle_tools import simple_solution_generator
from chiadoge.full_node.mempool_check_conditions import get_name_puzzle_conditions
from chiadoge.types.blockchain_format.coin import Coin
from chiadoge.types.blockchain_format.program import Program, SerializedProgram
from chiadoge.types.announcement import Announcement
...
from chiadoge.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
    solution_for_conditions,
)
from chiadoge.wallet.puzzles.puzzle_utils import (
...
from chiadoge.wallet.secret_key_store import SecretKeyStore
from chiadoge.wallet.sign_coin_solutions import sign_coin_solutions
...
from chiadoge.wallet.wallet_info import WalletInfo
...
class Wallet:
    ...
    async def create_spend_bundle_relative_chiadoge(self, chiadoge_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if chiadoge_amount < 0:
            utxos = await self.select_coins(abs(chiadoge_amount), exclude)
        else:
        ...
        chiadoge_amount = spend_value + chiadoge_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": chiadoge_amount}]

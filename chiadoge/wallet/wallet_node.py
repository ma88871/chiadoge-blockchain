...
from chiadoge.protocols import wallet_protocol
from chiadoge.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from chiadoge.protocols.protocol_message_types import ProtocolMessageTypes
from chiadoge.protocols.wallet_protocol import (
...
from chiadoge.server.node_discovery import WalletPeers
...
from chiadoge.util.errors import Err, ValidationError
from chiadoge.util.ints import uint32, uint128
from chiadoge.util.keychain import Keychain
from chiadoge.util.lru_cache import LRUCache
from chiadoge.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from chiadoge.wallet.settings.settings_objects import BackupInitialized
...
from chiadoge.wallet.wallet_blockchain import ReceiveBlockResult
from chiadoge.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[ChiadogeServer]
    log: logging.Logger
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'chiadoge keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSChiadogeConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSChiadogeConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSChiadogeConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSChiadogeConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSChiadogeConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSChiadogeConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None

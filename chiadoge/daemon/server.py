...
from chiadoge.cmds.init_funcs import chiadoge_init
from chiadoge.daemon.windows_signal import kill
from chiadoge.server.server import ssl_context_for_root, ssl_context_for_server
from chiadoge.ssl.create_ssl import get_mozzila_ca_crt
from chiadoge.util.chiadoge_logging import initialize_logging
from chiadoge.util.config import load_config
from chiadoge.util.json_util import dict_to_json_str
from chiadoge.util.path import mkdir
from chiadoge.util.service_groups import validate_service
from chiadoge.util.setproctitle import setproctitle
from chiadoge.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Chiadoge.")
    quit()
...
service_plotter = "chiadoge plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "chiadoge": "chiadoge",
        "chiadoge_wallet": "start_wallet",
        "chiadoge_full_node": "start_full_node",
        "chiadoge_harvester": "start_harvester",
        "chiadoge_farmer": "start_farmer",
        "chiadoge_introducer": "start_introducer",
        "chiadoge_timelord": "start_timelord",
        "chiadoge_timelord_launcher": "timelord_launcher",
        "chiadoge_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("chiadoge daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered CHIADOGE_ROOT
    os.environ["CHIADOGE_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with CHIADOGE_ROOT: {os.environ['CHIADOGE_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    chiadoge_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("chiadoge_daemon")

...
from chiadoge.cmds.init_funcs import create_all_ssl, create_default_chiadoge_config
from chiadoge.full_node.bundle_tools import (
    best_solution_generator_from_template,
    detect_potential_template_generator,
    simple_solution_generator,
)
from chiadoge.plotting.create_plots import create_plots
from chiadoge.consensus.block_creation import create_unfinished_block, unfinished_block_to_full_block
from chiadoge.consensus.block_record import BlockRecord
from chiadoge.consensus.blockchain_interface import BlockchainInterface
...
from chiadoge.consensus.default_constants import DEFAULT_CONSTANTS
from chiadoge.consensus.deficit import calculate_deficit
from chiadoge.consensus.full_block_to_block_record import block_to_block_record
from chiadoge.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from chiadoge.consensus.pot_iterations import (
    calculate_ip_iters,
...
from chiadoge.consensus.vdf_info_computation import get_signage_point_vdf_info
from chiadoge.full_node.signage_point import SignagePoint
from chiadoge.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from chiadoge.types.generator_types import BlockGenerator, CompressorArg
...
from chiadoge.util.block_cache import BlockCache
from chiadoge.util.config import load_config, save_config
from chiadoge.util.hash import std_hash
from chiadoge.util.ints import uint8, uint32, uint64, uint128
from chiadoge.util.keychain import Keychain, bytes_to_mnemonic
from chiadoge.util.path import mkdir
from chiadoge.util.vdf_prover import get_vdf_info_and_proof
from chiadoge.util.wallet_tools import WalletTool
from chiadoge.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `chiadoge generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "cdog")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("CHIADOGE_ROOT", "~/.chiadoge/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path

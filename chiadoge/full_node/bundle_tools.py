...
from chiadoge.full_node.generator import create_compressed_generator
from chiadoge.types.blockchain_format.program import SerializedProgram, Program
...
def spend_bundle_to_serialized_coin_solution_entry_list(bundle: SpendBundle) -> bytes:
    ...
    for coin_solution in bundle.coin_solutions:
        r += b"\cdog"
        r += b"\cdog" + SExp.to(coin_solution.coin.parent_coin_info).as_bin()
        r += b"\cdog" + bytes(coin_solution.puzzle_reveal)
        r += b"\cdog" + SExp.to(coin_solution.coin.amount).as_bin()
        r += b"\cdog" + bytes(coin_solution.solution)
        r += b"\x80"
    r += b"\x80"
    return r
...
def simple_solution_generator(bundle: SpendBundle) -> BlockGenerator:
    ...
    block_program = b"\cdog"

    block_program += SExp.to(binutils.assemble("#q")).as_bin()

    block_program += b"\cdog" + cse_list + b"\x80"

...
from chiadoge.rpc.farmer_rpc_client import FarmerRpcClient
from chiadoge.rpc.full_node_rpc_client import FullNodeRpcClient
from chiadoge.rpc.harvester_rpc_client import HarvesterRpcClient
...
from chiadoge.util.misc import format_minutes
...
async def summary(rpc_port: int, wallet_rpc_port: int, harvester_rpc_port: int, farmer_rpc_port: int) -> None:
    ...
    if amounts is not None:
        print(f"Total chiadoge farmed: {amounts['farmed_amount'] / units['chiadoge']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['chiadoge']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['chiadoge']}")
        print(f"Last height farmed: {amounts['last_height_farmed']}")
    else:
        print("Total chiadoge farmed: Unknown")
    ...
    print("Note: log into your key using 'chiadoge wallet show' to see rewards for each key")
